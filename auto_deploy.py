...
# 请在配置文件中设置或使用环境变量
SERVER_CONFIG = {
    'hostname': os.getenv('DEPLOY_HOST', 'your_server_ip'),
    'username': os.getenv('DEPLOY_USER', 'root'),
    'password': os.getenv('DEPLOY_PASS', 'your_password'),
    'port': 22,
}
...
def main():
    ...
    try:
        ...
        # 配置环境变量
        print_colored("\n配置环境变量...", 'cyan')
        print_colored("⚠️  请手动配置环境变量文件 .env", 'yellow')
        print_colored("   参考 .env.example 文件配置您的密钥", 'yellow')
        print_colored("   配置完成后运行: python3 daemon.py start", 'yellow')
        
        # 检查是否已有守护进程运行
        print_colored("\n检查守护进程状态...", 'cyan')
        execute_command(ssh, f"cd {REMOTE_PATH} && python3 daemon.py status", show_output=True)
        
        # 询问是否启动
        print()
        print_colored("部署完成！", 'green')
        print()
        print_colored("需要启动服务吗？", 'yellow')
        print("1. 立即启动守护进程")
        print("2. 稍后手动启动对战进程")
        choice = input("\n请选择 (1/2): ").strip()
        ...
        if choice == '1':
            print_colored("\n启动守护进程...", 'cyan')
            execute_command(ssh, f"cd {REMOTE_PATH} && python3 daemon.py start")
            
            print()
            execute_command(ssh, f"cd {REMOTE_PATH} && python3 daemon.py status")
            print()
            ...
            print()
            print_colored("手动启动命令：", 'yellow')
            print(f"  cd {REMOTE_PATH}")
            print("  python3 daemon.py start")
        print()
